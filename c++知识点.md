...
链接 生成可执行文件（如 Windows 的`.exe`、Linux 的 ELF 文件）将多个目标文件（及所需的库文件）合并为一个**可执行文件**，解决代码中的 “外部引用”（如调用其他文件的函数、使用库中的变量）。
...
在链地址法中，每个哈希表的槽（即数组的位置）都包含一个链表（或其他数据结构，如平衡树、动态数组等）。哈希数组是一个动态扩容的桶数组，每个元素（“桶”）是一个**链表 / 红黑树**的头指针（不同标准库实现可能不同，如 libstdc++ 用链表，libc++ 用更高效的结构）。当发生冲突时，将所有具有相同哈希值的元素（即具有相同数组索引的元素）链接在一起。这种方法的优势是不会影响其他槽的使用，甚至可以解决**严重的哈希冲突**。
...
### heap && priority_queue（容器适配器）

heap（堆） ：

建⽴在完全⼆叉树上，分为两种，大根堆，小根堆。完全二叉树（Complete Binary Tree）是一种特殊的二叉树结构，其核心特征是 **“除最后一层外，每一层都被完全填满，且最后一层的节点从左到右连续排列，没有间隔”**。

在STL中做priority_queue的助⼿，即，以任何顺序将元素推⼊容器中，然后**取出时⼀定是从优先权最⾼的元素开始取**，完全⼆叉树具有这样的性质，适合做priority_queue的底层

priority_queue，底层由数组vector实现。

**默认排序**：使用最大堆（最大优先队列），compare（）>为最小堆
...
二叉搜索树的每个节点包含一个 “键值（Key）”，且满足以下严格规则：

1. **左子树**中所有节点的键值**小于**当前节点的键值；
2. **右子树**中所有节点的键值**大于**当前节点的键值；
3. **左右子树本身也必须是二叉搜索树**（递归定义）；
4. （通常约定）树中**不允许重复键值**（若需支持重复值，可调整规则为 “左子树≤当前节点” 或 “右子树≥当前节点”）。

平衡 BST 是指**通过特定机制主动维持左右子树高度平衡**的二叉搜索树。其核心目标是将树的高度严格控制在 **O(log n)**，确保所有操作的时间复杂度稳定在 O (log n)。非平衡则是未主动维持平衡。

1. 平衡的标准

不同平衡树对 “平衡” 的定义不同，常见标准：

- **AVL 树**：左右子树的高度差（平衡因子）**不超过 1**（最严格的平衡）。
- **红黑树**：通过 “颜色规则”（如根黑、叶黑、红节点子节点必黑等）间接维持平衡，左右子树高度差**不超过 2 倍**（相对宽松）。

2. 平衡的实现：调整机制

为维持平衡，插入或删除节点后，平衡树会通过**旋转操作**（或其他调整）修正树的结构。以 AVL 树为例，常见旋转包括：

- **单旋**（左旋转、右旋转）：用于修正简单的不平衡。
- **双旋**（左右旋转、右左旋转）：用于修正复杂的不平衡。
...
是一种自平衡的二叉搜索树（BST），它通过在每个节点上增加一个 “颜色” 属性（红色或黑色），并通过一系列规则（旋转和重新着色）维护树的平衡性，确保其操作（插入、删除、查找）的时间复杂度稳定在O(log n)（n 为节点数）。
一、红黑树的核心性质
红黑树的平衡性由以下 5 条严格规则保证，任何操作后都必须维持这些性质：
颜色规则：每个节点要么是红色，要么是黑色。
根规则：根节点必须是黑色。
叶子规则：所有叶子节点（NIL 节点，即空节点）必须是黑色。
（注：实际实现中，叶子节点通常被统一表示为一个哨兵节点NIL，简化边界处理）
红父规则：如果一个节点是红色，则它的两个子节点必须是黑色（即不允许两个红色节点直接相邻）。
黑高规则：从任意节点到其所有叶子节点的路径中，包含的黑色节点数量必须相等（称为 “黑高”）。
二、性质的作用：保证平衡性
这些规则从根本上限制了红黑树的 “倾斜程度”：
由规则 4 和 5 可推导出：红黑树中最长路径（红黑交替）的长度不会超过最短路径（全黑）的 2 倍。
因此，红黑树的高度始终是O(log n)，避免了普通 BST 在极端情况下退化为链表（O (n) 复杂度）的问题。
三、红黑树的结构
红黑树的节点结构在 BST 基础上增加了颜色属性，示例如下：
...
struct RBNode {
    ...
    RBNode* left;     // 左子节点
    RBNode* right;    // 右子节点
    RBNode* parent;   // 父节点（红黑树需要父指针辅助旋转）
    bool is_red;      // 颜色：true为红，false为黑
    RBNode(Key k, Value v) : key(k), value(v), left(nullptr), right(nullptr), parent(nullptr), is_red(true) {}
};
```

新插入的节点默认是红色（减少对 “黑高规则” 的破坏，因为插入黑色节点会直接改变路径的黑高）。
所有空指针（叶子）被视为NIL节点（黑色），统一处理边界情况。
四、核心操作：插入与平衡调整
插入操作分为两步：1. 按 BST 规则插入新节点（设为红色）；2. 通过旋转和着色修复红黑树性质。

1. 插入后可能破坏的性质
新节点为红色，若其父节点也是红色（违反规则 4：红父不能有红子），需要调整。此时祖父节点必为黑色（否则插入前已违反规则 4），关键看 “叔叔节点”（父节点的兄弟）的颜色：
2. 调整策略（以父节点是祖父左子为例）
情况 1：叔叔节点是红色
操作：将父节点和叔叔节点改为黑色，祖父节点改为红色。
目的：消除 “红父红子” 冲突，同时不改变黑高（原路径黑节点数量不变）。
可能引发的问题：祖父节点变红后，若其父亲也是红色，需递归向上调整。
情况 2：叔叔节点是黑色（或 NIL），且新节点是父节点的右子
操作：先对父节点进行左旋，将新节点变为父节点的左子（转化为情况 3）。
情况 3：叔叔节点是黑色（或 NIL），且新节点是父节点的左子
操作：
将父节点改为黑色，祖父节点改为红色；
对祖父节点进行右旋。
目的：通过旋转改变结构，同时修复颜色冲突，且不改变黑高。
五、核心操作：删除与平衡调整
删除操作更复杂，步骤为：1. 按 BST 规则删除节点；2. 修复可能被破坏的红黑树性质。
1. 删除节点的分类
若删除的是红色节点：直接删除（通常不破坏性质）。
若删除的是黑色节点：可能破坏 “黑高规则”（路径黑节点减少）或 “红父规则”，需调整。
2. 调整核心：处理 “双重黑色”
删除黑色节点后，其位置会产生 “双重黑色”（可理解为路径黑高需要补偿），需通过以下策略消除：
兄弟节点是红色：先旋转将兄弟变为黑色，祖父变为红色，转化为兄弟是黑色的情况。
兄弟节点是黑色，且兄弟的两个子节点都是黑色：将兄弟改为红色，“双重黑色” 向上传递给父节点，递归处理。
兄弟节点是黑色，且兄弟的右子是红色（左子任意）：旋转 + 着色，消除双重黑色（兄弟变红，右子变黑，祖父继承兄弟原颜色）。
