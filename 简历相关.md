...
![image-20250910165134711](E:\找工作\八股\interview_summary-main\image\简历相关_image\新快捷方式.lnk)

空间误差补偿模块需要接受给定的输入最终计算出补偿值，让运动轴的指令位 置发生变动，这说明补偿功能在系统内是系统运动规划的一环，在内核态中完成，它 可以根据空间误差补偿表实时计算生成并输出空间误差补偿值。补偿值的生成周期 与机床插补周期保持一致，均为 1ms，是一种强实时性的补偿，可以对机床进给轴的 指令位置变化产生毫秒级的响应。

![image-20250910170058388](E:\找工作\八股\interview_summary-main\image\简历相关_image\image-20250910170058388.png)
...
在数控系统中，指令位置数据（如各轴实时位置）与插补模块的联调需解决 “跨线程 / 进程数据共享” 和 “实时同步” 两大核心问题。POSIX 共享内存（Shared Memory）是高效的进程间通信方式（内存级访问速度），结合线程同步机制（互斥锁 + 条件变量）可实现低延迟数据交互。以下是具体实现方案（基于 C 语言，适配 Linux 实时内核环境）：这里其实是插补模块与补偿模块的数据共享。

##### 一、核心设计思路

1. **共享内存载体**：创建 POSIX 共享内存区域，存储 “指令位置数据”+“同步状态标记”，供插补模块（生产者）和读取线程（消费者，如误差补偿模块）共享访问；
2. **同步机制**：用 POSIX 互斥锁（`pthread_mutex_t`）保证数据读写互斥，用条件变量（`pthread_cond_t`）实现 “数据就绪通知”，确保读取线程仅在新数据产生时才处理（避免空轮询浪费 CPU）；
3. **实时性优化**：设置线程优先级（插补模块为高优先级实时线程），共享内存页锁定（`mlock`）防止换出到磁盘，保证微秒级响应。

#### 匹配 VCS 表中的误差项标识（如`[EXX]`、`[EYX]`）

VCS 表中误差项以`[E+字母+字母]`格式标识（如`[EXX]`表示 X 轴 X 方向误差），需用正则提取这些标识并校验格式，const regex vcs_pattern(R"(\[E([A-Z])([A-Z])\])"); 

VCS 表中误差数据行由 “指令位置 + 误差值” 组成（空格分隔的浮点数），需用正则提取并校验：const std::regex data_pattern(R"(^[-+]?\d+(\.\d+)?\s+[-+]?\d+(\.\d+)?$)");

#### 伺服通讯协议

在伺服协议通信中，结构体的内存布局和对齐方式直接影响数据传输的准确性（尤其是底层硬件寄存器交互或二进制协议解析场景）。在 VCS 误差补偿项目中，伺服协议的准确性直接影响补偿值的下发（如将计算后的误差补偿值通过协议发送给伺服驱动器），用`_Static_assert`校验了协议结构体的对齐和大小，确保补偿指令通过 CRC 校验时不会因内存填充导致校验失败，

### 华中数控系统机床几何误差自动采集软件开发 2024年10月 - 2025年02月 

项目介绍: 解决传统人工测量机床几何误差效率低、数据不可追溯的问题，开发智能采集软件。

#### XML 配置

是一种使用 XML（可扩展标记语言）格式存储应用程序设置、参数或结构定义的方式，因其结构化、可读性强且易于解析的特点，被广泛用于各类软件、框架（如 Spring、MyBatis 等）的配置中。

结构说明：

- `<?xml version="1.0" encoding="UTF-8"?>` 是 XML 声明，指定版本和编码
- `<!-- 注释内容 -->` 是 XML 注释
- 所有内容需包含在一个根元素中（这里是 `<configuration>`）
- 元素可以嵌套（如 `<application>` 包含 `<description>`）
- 元素可以有属性（如 `name="MyApp"`、`enabled="true"`）
...
<scale_config>
    ...
    <scenarios>
        <scenario id="scenario1">
            <!-- 温度串口号配置 -->
            <com_port>COM8,COM9</com_port>
            <!-- 量表串口号配置 -->
            <LBcomport>COM8,COM9</LBcomport>
            <!-- 采样模式配置：manual(手动)/ontime(定时)/onposition(G代码定位) -->
            <sampling_mode>ontime</sampling_mode>
            <!-- 温度采样板卡从机起始地址-->
            <startaddress>0,200</startaddress>
            <!-- 温度采样波特率-->
            <baudrate>9600</baudrate>
            <!-- 温度采样点数,采集系统温度寄存器只需填写寄存器数，使用温度采集板卡需填写各个板卡的温度采集数量-->
            <pointnums>1,2</pointnums>
            <!-- 定时模式下采样间隔 -->
            <collect_interval>1000</collect_interval>
            <!-- 文件保存路径及命名：数字位置1：采集轴配置即对应量表为xzy轴即为021，2：温度配置数，3：文件标识-->
            <save_path>./Axis_Thermal_Err_21_2_1.csv</save_path>
            <!-- 测试G代码模板-->
            <gcode_template>%G0 X10 Y10\nG1 Z5 F200</gcode_template>
            <!-- ui按键名称配置-->
            <ui_pushbuttonname>配置查看,连接量表,导入G代码,手动采集,定时采集,删除采样点,数据保存</ui_pushbuttonname>
            <!-- ui按键隐藏-->
            <ui_pushbuttonhide>手动采集</ui_pushbuttonhide>
            <!-- 测量数据项名称配置-->
            <dataname>3080,3081,时间</dataname>
            <!-- 基准轴号-->
            <baseaxis>0</baseaxis>
            <!-- 测温类型: 0(温度采集板卡)1(温度寄存器)2(襄阳采集卡)-->
            <tempcollecttype>1</tempcollecttype>
            <!-- 误差测量类型: 0(marh)1(量仪)2(英示)3（双向量仪）4（三向量仪）-->
            <datacollecttype>0</datacollecttype>
            <!-- 温度寄存器序列-->
            <tempreg>G3080,G3081</tempreg>
            <!-- 采集类型：0（主轴）1（丝杆）2（测温点筛选）-->
            <type>1</type>
            <!-- 采集位置-->
            <collectposition>0,0,0 1,1,1</collectposition>
            <!-- 丝杆补偿轴号-->
            <sigan>0</sigan>
        </scenario>
        <scenario id="scenario2">
            <com_port></com_port>
            <LBcomport>COM10</LBcomport>
            <sampling_mode>ontime</sampling_mode>
            <startaddress>0</startaddress>
            <baudrate>9600</baudrate>
            <pointnums>2</pointnums>
            <collect_interval>1000</collect_interval>
            <save_path>./Axis_Thermal_Err_1_2_1.csv</save_path>
            <gcode_template>%G0 X10 Y10\nG1 Z5 F200</gcode_template>
            <ui_pushbuttonname>配置查看,连接设备,导入G代码,手动采集,数据保存</ui_pushbuttonname>
            <ui_pushbuttonhide></ui_pushbuttonhide>
            <dataname>误差值1,3080,3081,时间/s</dataname>
            <tempcollecttype>1</tempcollecttype>
            <datacollecttype>1</datacollecttype>
            <tempreg>3080,3081</tempreg>
            <type>0</type>
            <collectposition>0,0,100</collectposition>
            <sigan>2</sigan>
        </scenario>
        <scenario id="scenario3">
            <com_port>COM11</com_port>
            <sampling_mode>manual</sampling_mode>
            <sampling_points>10</sampling_points>
            <start_point>(0,0,0)</start_point>
            <end_point>(10,10,5)</end_point>
            <collect_interval>1000</collect_interval>
            <save_path>./data_scenario3.csv</save_path>
            <gcode_template>%G0 X10 Y10\nG1 Z5 F200</gcode_template>
            <ui_pushbuttonname>配置查看,连接量表,导入G代码,手动采集,定时采集,删除采样点,数据保存</ui_pushbuttonname>
            <ui_pushbuttonhide>定时采集</ui_pushbuttonhide>
            <dataname>PositionX,PositionY,PositionZ,Data1</dataname>
            <baseaxis>0</baseaxis>
<type>0</type>
            <tempcollecttype>1</tempcollecttype>
            <datacollecttype>1</datacollecttype>
...
#### HTTP（超文本传输协议）和 HTTPS（超文本传输安全协议）

是用于在网络上传输数据的应用层协议，二者核心功能相似，但在安全性上有本质区别。以下从定义、区别、工作原理等方面详细说明：

#### 1. 基本定义

- **HTTP（HyperText Transfer Protocol）**
  一种无状态的应用层协议，用于客户端（如浏览器）与服务器之间传输超文本（如 HTML、图片、视频等）。它基于 TCP 协议，默认使用 80 端口，**数据以明文形式传输**，不提供加密和身份验证。
- **HTTPS（HyperText Transfer Protocol Secure）**
  是 HTTP 的安全版本，在 HTTP 基础上加入了 SSL/TLS（安全套接层 / 传输层安全）协议进行加密和身份验证。默认使用 443 端口，**所有数据传输经过加密**，能防止窃听、篡改和伪造。
...
| 证书需求   | 无需证书            | 需要 CA（证书颁发机构）颁发的 SSL 证书 |
| 性能开销   | 较低（无加密过程）  | 较高（需加密 / 解密、握手过程）        |
| 浏览器提示 | 可能标记为 “不安全” | 地址栏显示锁形图标（安全标识）         |

#### 3. HTTPS 工作原理（SSL/TLS 握手过程）

HTTPS 的安全核心是 SSL/TLS 协议，其通过 “握手过程” 建立加密连接，大致流程如下：

1. **客户端请求**：浏览器向服务器发送 HTTPS 请求，携带支持的加密算法列表。
2. **服务器响应**：服务器选择合适的加密算法，返回 SSL 证书（包含公钥和服务器身份信息）。
3. **证书验证**：浏览器验证证书合法性（是否由可信 CA 颁发、是否过期等），若验证通过，生成一个随机的 “会话密钥”。
4. **密钥加密传输**：浏览器用服务器的公钥加密 “会话密钥”，发送给服务器。
5. **建立加密连接**：服务器用自己的私钥解密得到 “会话密钥”，此后双方用该密钥进行对称加密通信（效率更高）。

#### 4. 应用场景

- **HTTP**：适用于对安全性要求极低的场景（如内部非敏感数据传输），但目前已被主流浏览器标记为 “不安全”，实际使用极少。
- **HTTPS**：几乎所有公开网站（尤其是涉及用户隐私、支付、登录的场景，如电商、银行、社交平台）均需使用，是现代网络的基本安全标准。

#### 5. 为什么需要 HTTPS？

- **防窃听**：加密数据可防止第三方（如黑客、运营商）获取传输内容（如密码、支付信息）。
- **防篡改**：通过数字签名确保数据在传输中未被修改（如防止黑客篡改购物车价格）。
- **防伪造**：SSL 证书可验证服务器身份，防止用户访问钓鱼网站（如伪装成银行的虚假网站）。

#### 总结

HTTPS 并非全新协议，而是 “HTTP + SSL/TLS” 的组合，通过加密和身份验证解决了 HTTP 的安全缺陷。随着网络安全意识的提升，HTTPS 已成为互联网的标配，几乎所有主流网站都已采用。
