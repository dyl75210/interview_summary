...
1. **脏位（Dirty Bit）**：每个 Cache Block 必备的 1 位标志，用于记录该 Block 是否被 CPU 修改过。只有当脏位为 1 时，替换时才需要写回内存（避免无意义的内存写入）。
2. **替换算法**：决定替换哪个 Block（如 LRU 记录最近使用时间，优先替换最久未用的 Block），核心是 “保留更可能被再次访问的数据”，减少后续 miss 率。
3. **数据一致性**：写回策略通过 “替换时强制写回脏数据” 保证 Cache 与内存的最终一致性，但过程中存在短暂不一致（Cache 修改后、未写回内存前）。因此，在多核心 / 多处理器系统中，需配合**缓存一致性协议（如 MESI）** 解决跨核心的数据同步问题。
